...
**患者データは端末から出ない。** 通信を行わず、保存もしない（`connect-src 'none'` を
CSP で固定）。データが外に出る経路は、検者が明示的に実行するファイル書き出しだけである。

- 動くもの: <https://yamaguchitoshi.github.io/mnread/>
- 仕様版 0.6.0 / アルゴリズム版 0.5.0（画面と全出力に表示される）
...
| テスト | 538件（`packages/core` 378 / `apps/web` 160）すべて通過 |
...
詳細と経緯は `docs/adr/`（16件）にある。とくに効いているもの:
...
apps/web/          React + 自前SVG。外部CDNに依存しない
```
