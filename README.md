...
- 仕様版 0.6.3 / アルゴリズム版 0.5.0（画面と全出力に表示される）
...
`references/` の公式マニュアルと Q&A から転記したゴールデンデータに対して、`pnpm verify:fixtures`
...
照合対象は R パッケージ1本にとどまらない。Calabrèse et al. (2018, J Vis 18(1):8) が、
MNREAD iPad アプリの自動推定は `mnreadR` と同じ計算（Legge 2007 法）だと明記している
（`references/i1534-7362-18-1-8.pdf`）。同論文は、その自動推定が目視で誤りと判定された割合を
ロービジョンで 9〜11% と報告しており、`requiresReview` の発火率を較正するときの外部の目安になる。
...
```
references/        文献（一次資料の公式マニュアル・Q&A、および参考文献の PDF）
...
**将来の可能性** — 対応端末として iPad を考慮する余地がある（`PLAN.md` Phase 6）。入力は
既に数字キーパッドを出す作りになっており障害にならない。未確認なのは印刷で、iPadOS には
用紙と倍率を指定する手立てがなく、実物大の見本が紙の上で正しい大きさになるかは未測定である。
着手するとしても、まず現場の iPad で 50 mm の目盛りを測るところから始める。
...
| `references/` | 文献。一次資料（正本）と参考文献。式の疑義は調査報告ではなくここに当たる |
