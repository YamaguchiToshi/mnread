...
- **The published build's one external request is the analytics beacon, and the CSP hole exists only where the beacon does** (ADR-0017). `index.html` stays at `connect-src 'none'`; `apps/web/vite/web-analytics.ts` opens exactly two Cloudflare hosts at build time, and only when `VITE_CF_BEACON_TOKEN` is set. The on-screen "外部への通信" wording in `UsageNotes.tsx` is driven by the same variable — if you change one, the other must move with it, or the app tells clinicians something false about the build they are looking at. Do not widen the hole for anything else; nothing patient-derived may ride on it.
...
**Phases 0–4 are complete.** 557 tests pass (core 378 + web 179).
...
- **`@page` keeps a 10 mm margin and `.report`'s print padding supplies the rest** (20/15/10 mm in total, 180 mm text block). Zero page margin does suppress the browser's own header/footer — they are printed inside the page margin — but a full-bleed page box exceeds the printer's unprintable border, and the print pipeline then scales the whole sheet down to fit: measured, the 50 mm calibration bar came out at 47 mm. That makes the actual-size specimen not actual size, and silently disarms the very bar that exists to detect shrink-to-fit printing. Physical fidelity wins; the header/footer is switched off in the print dialog instead, and `OutputPanel` says so on screen.
...
**The specimen's font is bundled; everything else on the sheet uses the machine's fonts** (ADR-0018). The specimen and the 50 mm bar are measuring instruments — they assert a physical size — and `pt` fixes the em, not the printed ink: the ink is the face's 字面率 times the em (Hiragino Mincho ProN W3's 読 is 0.913 em, the bundled Noto Serif JP subset 0.918). A fallback stack therefore prints a different-sized specimen on a Mac than on a Windows box, and **the calibration bar cannot see it** — the bar catches scaling, and scaling doesn't move 字面率. So `.specimen-text` names a bundled one-glyph subset first (`src/assets/mnread-report-mincho.woff2`, ~1 KB, regenerated by `scripts/build-specimen-font.py`; OFL text ships in `public/`). The report's other text asserts nothing physical and needs no font. Never rescale the glyph to match some other face's ink — the label says "N pt", so the em is what must be N pt. If you swap the font, print it and measure: the browser number to reproduce is 0.9179 em at `emulateMedia("print")`.

`docs/report-sample.png` is the printed reference: an A4 scan of the sheet at 300 dpi, from the synthetic session in `apps/web/src/sample/sampleSession.ts` (`?sample` on a dev build, which also stamps the SAMPLE watermark — the data and the watermark share one switch, so neither can appear without the other, and the switch folds to `false` in the published build). It was measured, not eyeballed: text block 180.09 mm against a 180 mm design, calibration bar 49.95 mm against 50 mm. When you change the report, print it again and measure — a print that looks right can still be scaled, and the bar is what tells you.

**Nothing on the sheet may overflow the page box — Chrome answers overflow by shrinking the whole sheet.** The `SAMPLE` watermark was an `inset: 0` box carrying `transform: rotate(-30deg)`: rotating a 180×267 mm box gives a 289 mm bounding box, and Chrome scaled the printed page to 0.72 to fit it. Every physical dimension went with it — the actual-size specimen and the 50 mm calibration bar included — while `.report`'s `width: auto` refilled the page, so the sheet still looked full and only the measured bar gave it away (36.07 mm). Rotate the text, never the full-size box, and keep `overflow: hidden` on the un-rotated container. The same trap waits for any wide element: a long unbroken string, a fixed-width table, an oversized SVG.

**The sheet is true-size on A4 paper only, and the page cannot enforce that.** `@page { size: A4 portrait }` sets the page box; the paper in the dialog comes from the OS/printer default, and a mismatch makes the driver scale the whole sheet by the A4/Letter ratio — measured, the 50 mm bar came out at 47 mm in Chrome and 53 mm in Safari from Letter paper, in opposite directions. The control point is the machine's default paper size, so it belongs in `docs/review-request.md` and in the on-screen note next to the print button, not in CSS.
